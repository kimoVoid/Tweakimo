...
import me.kimovoid.tweakimo.util.Vector3f;
...
public class FreeCamController {
    ...
    public void onRenderTickStart() {
        if (active) {
            ...
            double slowdown = Math.pow(Tweakimo.CONFIG.freeCamFlySlowdownFactor, frameTime);
            forwardVelocity = combineMovement(forwardVelocity, forwardImpulse, frameTime, Tweakimo.CONFIG.freeCamFlyAcceleration, slowdown);
            leftVelocity = combineMovement(leftVelocity, leftImpulse, frameTime, Tweakimo.CONFIG.freeCamFlyAcceleration, slowdown);
            upVelocity = combineMovement(upVelocity, upImpulse, frameTime, Tweakimo.CONFIG.freeCamFlyAcceleration, slowdown);
            ...
            double maxSpeed = Tweakimo.CONFIG.freeCamFlyMaxSpeed;
            if (mc.gameSettings.keyBindSprint.getIsKeyPressed()) {
                maxSpeed *= Tweakimo.CONFIG.flySpeed;
            }
            if (speed > maxSpeed) {
