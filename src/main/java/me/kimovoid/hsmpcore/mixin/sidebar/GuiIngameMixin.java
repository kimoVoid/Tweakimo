package me.kimovoid.hsmpcore.mixin.sidebar;
...
public abstract class GuiIngameMixin extends Gui {
    ...
    public void func_96136_a(ScoreObjective objective, int x, int y, FontRenderer fr) {
        if (!HSMPCore.CONFIG.enableScoreboard || (HSMPCore.CONFIG.hideScoreboardInF3 && this.mc.gameSettings.showDebugInfo)) {
            return;
        }
        ...
        if (HSMPCore.CONFIG.scoreboardTotal) {
            int total = 0;
        ...
        for (Score score : collection) {
            ...
            String s2 = EnumChatFormatting.getValueByName(HSMPCore.CONFIG.scoreboardColor) + "" + score.getScorePoints();
    ...
    private void renderText(FontRenderer fr, String str, int x, int y, int color) {
        if (HSMPCore.CONFIG.scoreboardShadow) {
    ...
    private String getPlayerName(Team team, String playerName) {
        return ScorePlayerTeam.formatPlayerName(team, (HSMPCore.CONFIG.scoreboardHighlightName && playerName.equals(this.mc.thePlayer.getDisplayName()) ? EnumChatFormatting.BOLD + playerName : playerName));
    }
}